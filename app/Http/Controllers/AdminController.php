...
//    Category
    ...
    public function storeSubCategory(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $image_resize = Image::make(public_path('uploads/subcategory/'.$imageName));
...
//          'kit' => 'required',
            'kit' => 'nullable',
            'box_size_length' => 'required|numeric',
            'box_size_width' => 'required|numeric',
            'box_size_height' => 'required|numeric',
...
//          'measurements.*.shelf_distance' => 'required',
            ...
            'measurements.*.depth' => 'nullable|numeric',
            'measurements.*.height' => 'nullable|numeric',
            'measurements.*.diameter' => 'nullable|numeric',
            'measurements.*.depth_open' => 'nullable|numeric',
            'measurements.*.height_open' => 'nullable|numeric',
            'measurements.*.seat_width' => 'nullable|numeric',
            'measurements.*.seat_depth' => 'nullable|numeric',
            'measurements.*.seat_height' => 'nullable|numeric',
            'measurements.*.arm_height' => 'nullable|numeric',
            'measurements.*.desk_clearance' => 'nullable|numeric',
            'measurements.*.shelf_distance' => 'nullable|numeric',
//          'measurements.*.weight'=>'required',
            ...
            'components.*.box_weight' => 'nullable|numeric',
            'components.*.cubes' => 'nullable|numeric',
            'components.*.box_size_length' => 'nullable|numeric',
            'components.*.box_size_width' => 'nullable|numeric',
            'components.*.box_size_height' => 'nullable|numeric',
...
//                    'ProductNumber' => $product->ProductNumber,
            ...
            if ($request->hasFile('images')) {
                foreach ($request->file('images') as $image) {
                    ...
                    $image_resize = Image::make(public_path('uploads/product/'.$name));
                    $image_resize->resize(300, null, function ($constraint) {
                        $constraint->aspectRatio();
                    });
                    $image_resize->save(public_path('thumbnail/uploads/product/' . $name));
...
//                }
    ...
    public function getStyleByCoasterDistinct(Request $request)
    {
        $count = Style::whereNotNull('StyleCode')->select('StyleName')->groupBy('StyleName')->count();;
        $limit = Style::whereNotNull('StyleCode')->select('StyleName')->groupBy('StyleName')->count();
        $offset = 0;
        if (!empty($request->input('limit'))) {
            $limit = $request->input('limit');
        }
        ...
        $styles = Style::whereNotNull('StyleCode')->select('StyleName')->groupBy('StyleName')->offset($offset)->limit($limit)->orderBy('id', 'asc')->get();
